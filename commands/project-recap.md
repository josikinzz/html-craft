...
3. **Current state.** Check for uncommitted changes (`git status`). Check for stale branches (`git branch --no-merged`). Look for TODO/FIXME comments in recently changed files. Read project-local progress docs if they exist (`.pi/todos/`, `docs/`, `notes/`, or similar).
