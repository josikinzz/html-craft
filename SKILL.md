...
**Steer.** The page is where the user gets back into the work, so give them something to act on and a way to hand it back: options to choose between, a status to correct, a list to reorder, a value to tune, a claim to mark wrong. Timing is the lever — a page delivered after the work is finished can only be audited, while the same page at a decision point changes what happens next. When a decision is live, put the page in front of the user before you commit to an answer. The round-trip export is the strongest form of this: the user edits, exports, and the result re-enters the work as text (`./references/tool-patterns.md`). Any page that asks the user to approve or pick must let them reject the framing, not only choose among your options: free-text notes at page and item level, an escape from every choice set, a marked recommendation with its reason, and an export that reports an untouched field as unanswered rather than agreed. The full contract is "Asking the user" in `./references/tool-patterns.md`.

**Ask.** The conversation has its own channel, for decisions that can't wait for a page. A **fork** is a decision that changes what you build or what a plan commits to, and that the request, the conversation, and the material leave open — scope, audience, which of two readings, whether a risky step belongs in the plan. Settle a fork by asking, not by guessing: use the harness's structured question tool (AskUserQuestion in Claude Code; a plain-text question elsewhere), with options that are genuinely different answers and your recommendation listed first, marked, with its one-line reason — the same contract as "Asking the user" in `./references/tool-patterns.md`. Bundle the open forks into one call rather than a drip of interruptions. Everything else — aesthetic, fonts, structure, weight — is yours to decide, and most sessions have no fork at all. A fork whose options carry tradeoffs worth seeing side by side outgrows the question box: build the `explore-options` grid instead.
...
- **Steerable**: where a decision is still open, the reader has a way to act on it — options to pick between, a status to correct, an export that carries their edit back. A page that leaves them nothing to do but agree has skipped this. Where the page asks for a decision, it also meets "Asking the user" in `./references/tool-patterns.md`: notes fields at both scopes, an escape from every choice set, a reasoned recommendation, and an export that distinguishes confirmed, overridden, and untouched.
