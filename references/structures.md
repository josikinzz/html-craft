...
- **Spacing.** Two tiers, both from [`css-patterns.md`](css-patterns.md) → Spatial Scale. Space *between* elements — the gaps, margins, and card-to-card rhythm these recipes mostly deal in — comes from the rhythm scale: 4, 8, 12, 16, 24, 32, 48, 64, 96. Padding and tight gaps *inside* a component may take a half-step: 6, 10, 14, 20. Values under 4px are optical nudges and need no defence. Anything else is load-bearing geometry (a clip-path percentage, a ring radius, a fixed spoke width) and carries a comment saying so.
